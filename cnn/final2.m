...
% Last Modified by GUIDE v2.5 01-Mar-2020 17:39:58
...
axes(handles.axes1);
imshow(NDR);
title('Nucleus Regions');

%% Feature Extraction

% Statistical Color Feature Extraction
...
meanR = mean( R(:) );
set(handles.edit1,'string',meanR);
stdR = std( R(:) );
set(handles.edit2,'string',stdR);

meanG = mean( G(:) );
set(handles.edit3,'string',meanG);
stdG  = std( G(:) );
set(handles.edit4,'string',stdG);

meanB = mean( B(:) );
set(handles.edit5,'string',meanB);
stdB  = std( B(:) );
set(handles.edit6,'string',stdB);
...
axes(handles.axes5);
imshow(uint8(G));
title('Texture Features Extraction Result');

glcms = graycomatrix(G);

stats = graycoprops(glcms,'Contrast Correlation Energy Homogeneity');

CT = stats.Contrast;
set(handles.edit7,'string',CT);
CR= stats.Correlation;
set(handles.edit8,'string',CR);
EN = stats.Energy;
set(handles.edit9,'string',EN)
HG = stats.Homogeneity;
set(handles.edit10,'string',HG)


% Final Feature Vector

FF=[Mean Std CT CR EN HG];
